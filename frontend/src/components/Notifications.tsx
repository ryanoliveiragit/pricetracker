      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="absolute right-0 top-12 w-[380px] rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden shadow-[rgb(var(--primary-500))/0.10] z-[100]"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 bg-slate-50/50">
              <div className="flex items-center gap-2">
                <Bell className="h-4 w-4 text-[rgb(var(--primary-500))]" />
              ...
              {updates.map((update) => (
                ...
                >
                  <div className="mt-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white border border-slate-200 shadow-sm group-hover:scale-110 transition-transform">
                    <update.icon className="h-4 w-4 text-[rgb(var(--primary-500))]" />
                  </div>
                  <div className="flex-1 space-y-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[13px] font-bold text-slate-800 truncate tracking-tight">{update.title}</p>
                      {update.isNew && (
                        <span className="flex-shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold" style={{ background: "rgb(var(--primary-500) / 0.12)", border: "1px solid rgb(var(--primary-500) / 0.30)", color: "rgb(var(--primary-600))" }}>NOVO</span>
                      )}
                    </div>
            ...
            <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/50">
              <button 
                onClick={onClose}
                className="w-full rounded-lg py-2 text-center text-[11px] font-bold text-white transition-colors uppercase tracking-widest"
                style={{ background: "rgb(var(--primary-500))" }}
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgb(var(--primary-600))"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgb(var(--primary-500))"; }}
              >
                Entendi
              </button>
            </div>
