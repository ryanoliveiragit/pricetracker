...
export default function CommandMenu() {
  ...
  function handleSearch() {
    ...
    localStorage.setItem("construprice-last-search-items", JSON.stringify(finalSearch));
    window.dispatchEvent(new CustomEvent("construprice-header-search", { detail: finalSearch }));
    setIsOpen(false);
    setVariants([]);
    setInput("");
    router.push("/results");
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4">
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-lime-500 to-transparent opacity-50" />
            ...
            <div className="flex items-center px-4 py-4 border-b border-slate-100 dark:border-neutral-800 relative z-10 bg-white dark:bg-neutral-900">
               <Search className="h-5 w-5 text-lime-500 mr-3 hidden sm:block" />
               <div className="flex-1 flex flex-wrap gap-2 items-center min-w-0">
                  {variants.map((v) => (
                    <motion.span
                      ...
                      key={v} 
                      className="flex items-center gap-1.5 rounded-md bg-lime-100 dark:bg-lime-500/10 px-2 py-1 text-xs font-semibold text-lime-700 dark:text-lime-400 border border-lime-200 dark:border-lime-500/20 shadow-sm"
                    >
                      {v}
                      <button onClick={() => setVariants(variants.filter(item => item !== v))} className="hover:bg-lime-200 dark:hover:bg-lime-500/30 rounded-full p-0.5 transition-colors">
                        <X className="h-3 w-3" />
                      </button>
                    </motion.span>
                  ))}
                  <input
                    ...
                    placeholder={variants.length === 0 ? "Buscar produto ex: 'cola', 'tijolo'..." : "Adicionar outra variante..."}
               ...
               <div className="flex items-center gap-2 ml-3">
                 <button onClick={() => setIsOpen(false)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors">
                    <X className="h-5 w-5" />
                 </button>
               </div>
            </div>
            ...
            <div className="p-2 max-h-[60vh] overflow-y-auto bg-slate-50/50 dark:bg-neutral-900/50">
               {input.trim() && (
                 ...
                 >
                   <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-lime-100 dark:bg-lime-500/10 text-lime-600 dark:text-lime-500">
                     <Plus className="h-4 w-4" />
                   </div>
                   <div className="flex-1">
                     <p className="text-sm font-medium text-slate-700 dark:text-neutral-200">
                       Adicionar <span className="text-lime-600 dark:text-lime-400 font-bold">"{input.trim()}"</span> como variante
                     </p>
                     <p className="text-xs text-slate-400 dark:text-neutral-500 mt-0.5">Pressione Enter para adicionar rapidamente</p>
                   </div>
               ...
               {!input.trim() && variants.length === 0 && (
                 <div className="py-12 text-center">
                    ...
                    <p className="text-sm font-semibold text-slate-700 dark:text-neutral-300">Digite para buscar materiais</p>
                    <p className="text-xs text-slate-500 dark:text-neutral-500 mt-1 max-w-[280px] mx-auto">
                      Digite o nome de um produto e adicione quantas <b>variantes</b> ou <b>sinônimos</b> precisar para buscar tudo de uma vez.
                    </p>
                 </div>
               )}
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 dark:border-neutral-800 bg-slate-100 dark:bg-neutral-950 px-4 py-3">
               <div className="hidden sm:flex items-center gap-3 text-xs text-slate-500 dark:text-neutral-500">
                 <span className="flex items-center gap-1.5"><kbd className="rounded border border-slate-200 dark:border-neutral-700 px-1.5 py-0.5 bg-white dark:bg-neutral-800 shadow-sm">→</kbd> adicionar variante</span>
                 <span className="flex items-center gap-1.5"><kbd className="rounded border border-slate-200 dark:border-neutral-700 px-1.5 py-0.5 bg-white dark:bg-neutral-800 shadow-sm">Esc</kbd> fechar</span>
               </div>
               
               <button
                 onClick={handleSearch}
                 disabled={variants.length === 0 && !input.trim()}
                 className="flex flex-1 sm:flex-none items-center justify-center gap-2 rounded-xl bg-lime-500 px-6 py-2.5 text-sm font-semibold text-white transition-all hover:bg-lime-600 focus:ring-2 focus:ring-lime-500/50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm border border-lime-600/20"
               >
                 Buscar {variants.length > 0 ? `${variants.length + (input.trim() ? 1 : 0)} itens` : ""}
